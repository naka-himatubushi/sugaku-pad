...
```mermaid
flowchart TD
    A["手書き入力<br/>(Excalidraw / iPad PencilKit)"] -->|"PNG 化して送信"| R

    subgraph REC["認識層（確率的・ローカル LLM）"]
        R["POST /recognize"] --> V["Vision LLM<br/>qwen2.5-VL @ Ollama（端末内/LAN）"]
        V -->|"LaTeX 出力"| CL["デリミタ除去・整形"]
    end

    CL --> CARD["確認カード<br/>人が確認・修正<br/>（誤認識をここで吸収）"]
    CARD -->|"確定した LaTeX"| S["POST /solve"]

    subgraph CALC["計算層（決定的・SymPy）= mathai パッケージ"]
        S --> LI["LaTeX 入力層<br/>暗黙の掛け算 / frac・sqrt・三角 / 変数判定"]
        LI --> SE["SolveEngine<br/>一次・二次・式評価・三角・多項式 を分類して求解"]
        SE --> VF["検算レイヤー<br/>解を元の式へ代入し SymPy で成立確認 → verified"]
    end

    VF -->|"steps_latex / answer_latex / verified"| OUT["KaTeX 表示<br/>解答ドロワー（数式カード）＋ ✓検算済みバッジ"]

    classDef prob fill:#fff3e0,stroke:#e8a33d;
    classDef det fill:#e8f0fe,stroke:#5b8def;
    class REC prob;
    class CALC det;
```

- **認識層（オレンジ）= 確率的**: ローカル Vision LLM が手書き→LaTeX。完璧でない前提。
- **確認カード（人）**: 誤認識をワンタップで修正＝“最後の数%”の保険。
- **計算層（青）= 決定的**: SymPy で求解し、**答えを元の式に代入して検算**（`verified`）。間違いは弾ける。
