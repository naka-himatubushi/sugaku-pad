...
最終目標は **iPad 内で完結**（クラウドにも Mac にも依存しない）。

- 現状の認識は Mac 上の Ollama（qwen2.5-VL）。これを **iPad 単体で動かす**ために、
  最近発表された **[Apple Core AI](https://developer.apple.com/core-ai/)**（CoreML 後継の端末内 AI フレームワーク。コンパクトな vision モデル〜大規模生成 AI を端末内実行）を、**Ollama・クラウドの代替**として採用する方針。
- 現在は **API と変換可否を検証中**：OCR モデル（pix2tex エンコーダ）の **Core ML 変換に成功**（`torch.export` 経由）したのが初期の裏付け。Core AI への移行・端末内推論の検証を進めている。
...
- 🔬 **Apple Core AI による端末内推論を検証中**（Ollama / クラウドの代替）
