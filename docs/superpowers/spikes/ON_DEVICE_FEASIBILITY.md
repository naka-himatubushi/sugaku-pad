...
4. ✅ **iPad 実機で速度・RAM 実測済**（iPad Pro M4 8GB）: warm **2.38s** / RAM ピーク **3.2GB**（残 ~1.96GB）/ 認識 `2x+19=9` 正解。**8GB 機で成立**を実証（[OCR_BENCHMARK.md](OCR_BENCHMARK.md)）。Mac(0.23s) の約 10 倍だが確認カード UX なら実用域。
5. ⏳ 次: 速度詰め（解像度/プロンプト/より小さい VLM）＋ Canvas 手書き→MLX→確認カード→検算のフル統合。
